...
const Workspace = (props) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < elementDropped.length; i++) {
      console.log(dropped.length);
      if(dropped.length>1){
        const droppedIndex=dropped.findIndex(dropped=>elementDropped[i].position.x===dropped.position.x&&elementDropped[i].position.y===dropped.position.y);
