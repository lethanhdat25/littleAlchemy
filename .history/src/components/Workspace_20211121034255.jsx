...
const Workspace = (props) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < dropped.length; i++) {
      if(dropped.length>1){
        const droppedIndex=elementDropped.findIndex(elementDropped=>elementDropped.position.x===dropped[i].position.x&&elementDropped.position.y===dropped[i].position.y&&droppedIndex!==-1&&dropped[dropped.length-1].id===0&&dropped[dropped.length-1].name===elementDropped[droppedIndex].name);
