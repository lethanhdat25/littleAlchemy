...
const Library=(props)=>{ 
        ...
        if(props.newElement){
            setInitial(state=>{
                if(state.some(element=>element.name===props.newElement.name)){
                    return state;
                }else return state.concat(props.newElement);
    ...
    const renderInitialElement=()=>{
        const elements= initial.map((item)=>{
            return <Element id={item.id} type="inLibrary" key={item.id} name={item.name} className={cssElement.element} src={`/images/elements/${item.name}.png`} alt={item.name} />
        })
        return elements;
    }
    return(
