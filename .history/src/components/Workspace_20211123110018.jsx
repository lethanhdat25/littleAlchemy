...
const Workspace = (props) => {
  ...
  useEffect(() => {
    if(props.elementDelete){
       setElementDropped(state=>state.filter(element=>element.id!==props.elementDelete.id));
    }
  },[props.elementDelete])
  ...
  const addElement=(item,position)=>{
    setElementDropped((state) =>{
        if(state.length!==0&&item.type!=='inSpace'){
          ...
          if(isValid){
            indexElement++;
            return state.concat({ ...item, position, inSpace:"inSpace", id:item.name+"-"+indexElement})
          }
        }
        const elementFind=state.find(element=>element.id===item.id);
        const elementIndex=state.findIndex(element=>element.id===item.id);
        if(elementFind){
          const copElement={...elementFind,position};
          return state.map((element,index)=>{
            if(index===elementIndex) return {...element, position:copElement.position}
            return element;
          });
        }
  ...
  const mergeElement = (yourRecipe, yourPosition) => {
    ...
    const elementFonded = recipes.find( 
      (element) =>
        (element[2].name1 === recipe1 &&
          element[2].name2 === recipe2) ||
        (element[2].name1 === recipe2 &&
          element[2].name2 === recipe1)
    );
    if(elementFonded){
      const elementMerged = {
        id:elementFonded[0],
      ...
      setElementDropped((state)=>{
        const firstState= state.filter(element=>element.position.x!==position1.x&&element.position.y!==position1.y);
        const secondState=firstState.filter(element=>element.position.x!==position2.x&&element.position.y!==position2.y);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < elementDroppedLength; i++) {
      if (
        ...
        const yourPosition = [elementDropped[i].position,elementDropped[elementDroppedLength].position];
