...
const Workspace = (props) => {
  ...
  const [{item,position }, drop] = useDrop(() => ({
    ...
    drop: (item, monitor) => {
      ...
      setElementDropped((state) => item.id?state.concat({ ...item, position }):state.concat({ ...item, position,id:state.length}));
  ...
  useEffect(() => {
    setElements(elementDropped.map((element, index) => {
      const droppedIndex=dropped.findIndex(dropped=>element.position.x===dropped.position.x&&element.position.4===dropped.position.y);
