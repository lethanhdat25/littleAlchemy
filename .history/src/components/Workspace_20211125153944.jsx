...
const Workspace = (props) => {
  ...
  const mergeElement = (yourRecipe, yourPosition) => {
    ...
    if (elementFonded) {
      ...
      //add vào danh sách merge
      setElementMerged(state=>state.concat({...elementMerged,id:"merge"+"-"+elementMerged.name+"-"+Math.random().toString(36).substring(2, 15)}));
      
      //xóa các phần tử trước khi merge
      setElementDropped((state) => {
  ...
  useEffect(()=>{
    if (elementMerged.length>0) {
      ...
      const positioMerge=elementMerged[elementMerged.length-1].position;
      addElement(elementMerged[elementMerged.length-1],elementMerged[elementMerged.length-1].position)
    }
  },[elementMerged])
