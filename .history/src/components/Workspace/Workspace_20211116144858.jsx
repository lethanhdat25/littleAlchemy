...
const Workspace = () => {
  ...
  for (let i = 0; i < context.element.length - 1; i++) {
    if (
      Math.abs(context.element[i].position.x - lastElement.position.x) < 100 &&
      Math.abs(context.element[i].position.y - lastElement.position.y) < 100
    ) {
      ...
      if(resultCheck){
        ...
        context.removeElement(firstElement.name,secondElement.name);
      }
    }
  }
