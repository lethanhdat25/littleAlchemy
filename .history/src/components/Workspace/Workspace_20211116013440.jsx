...
const Workspace = () => {
  ...
  for (let i = 0; i < context.element.length - 1; i++) {
    if (
      ...
      resultCheck.current=recipes.find(
        (recipe) =>
      ...
      if(resultCheck.current){
        resultCheck.current.push(firstElement.position);
        console.log(resultCheck.current);
      }
    }
  }

  //Render element vào space
  const elements = !resultCheck.current?context.element.map((element, index) => {
    return (
      <Element
        name={element.name}
        src={element.src}
  ...
  <Element
        name={resultCheck.current[0]}
        src={`/images/elements/${resultCheck.current[0]}.png`}
        style={{
          position: "absolute",
          left:resultCheck.current[2].x,
          top: resultCheck.current[2].y,
        }}
      />
  return (
