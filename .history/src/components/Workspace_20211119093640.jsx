...
const Workspace = () => {
  ...
  let id=elementDropped[elementDropped.length-1]?.id+1;
  const [, drop] = useDrop(() => ({
    accept: ITEM_TYPE,
    drop: (item, monitor) => {
      const position = monitor.getClientOffset();
      setElementDropped((state) => state.concat({ ...item, position,id }));
      setDropped(item.id);
      id++;
    },
  }));
  ...
  const mergeElement = (yourRecipe, yourPosition, yourIndex) => {
    ...
    const elementMerged = {
      id:elementDropped[elementDropped.length-1].id+1,
      name: elementFonded[0],
      src: `/images/elements/${elementFonded[0]}.png`,
      position: yourPosition,
    };
    console.log(yourIndex);
    setElementDropped((state)=>{
      console.log(state);
      const firstState= state.filter(element=>element.id!==yourIndex[0]);
      console.log(firstState);
      const secondState=firstState.filter(element=>element.id!==yourIndex[1]);
