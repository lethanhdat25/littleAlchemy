...
import { useCallback, useDrag, useDrop } from "react-dnd";
...
const Element=(props)=>{
    ...
    const [{element},drag]=useDrag(()=>({ 
        ...
        type: ITEM_TYPE,
        collect:monitor=>{
            if(monitor.getItem()?.elementRef==="inSpace"){
                return {element:monitor.getItem()};
            }
        }
    }));
    const asd=useCallback(context.moveElement(element),[element])
    const [,drop]=useDrop(()=>({
        accept: ITEM_TYPE,
        drop: (item, monitor) => {
