    ...
    for (let i = 0; i < elementDropped.length - 1; i++) {
      if (
        ...
        const yourPosition = [
          ...
          elementDropped[elementDropped.length - 1].position,
        ];
    ...
    if (elementFonded) {
      ...
      setElementDropped((state) => {
        const firstState = state.filter(
          (element) =>
            element.position.x !== yourPosition[0].x &&
            element.position.y !== yourPosition[0].y
        );
        const secondState = firstState.filter(
          (element) =>
            element.position.x !== yourPosition[1].x &&
            element.position.y !== yourPosition[1].y
        );
    ...
    for (let i = 0; i < dropped.length; i++) {
      if (dropped.length > 1) {
        const droppedIndex = elementDropped.findIndex(
          (elementDropped) =>
            elementDropped.position.x === dropped[i].position.x &&
            elementDropped.position.y === dropped[i].position.y &&
            droppedIndex !== -1 &&
            dropped[dropped.length - 1].id === 0 &&
            dropped[dropped.length - 1].name ===
              elementDropped[droppedIndex].name
        );
