...
const Workspace = () => {
  ...
  useEffect(() => {
    for (let i = 0; i < elementDropped.length - 1; i++) {
      if (
        ...
        const yourIndex=[i,elementDropped.length - 1];
  ...
  const mergeElement = (yourRecipe, yourPosition, yourIndex) => {
    ...
    setElementDropped((state)=>{
      const fisrtState= state.slice(yourIndex[0],1);
      const secondState=fisrtState.slice(yourIndex[1],1);
