...
const AlchemyProvider=(props)=>{
    ...
    const initialValue={
        ...
        removeElement:(firstItem,secondItem)=>setAlchemy((state)=>{
            const fisrtFilter= state.findIndex(element=>element.name===firstItem.name&&elment.position.x===firstItem.position.x&&elment.position.y===firstItem.position.y);
            console.log(fisrtFilter);
